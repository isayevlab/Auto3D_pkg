...
is deliberate. ``Auto3D.entry.ASE.thermo`` used to reach a model four different ways
...
class ModelAdapter(Protocol):
    ...
    :class:`Auto3D.engines.batch_opt.model_wrapper.EnForce_ANI`. Be clear about what that
    ...
    def forward(
        ...
        Args:
            ...
            atom_mask: Boolean (batch, n_atoms), True for real atoms and False
                ...
                :func:`Auto3D.engines.batch_opt.padding.pad_from_mols`. Required from
    ...
    def analytic_hessian(
        ...
        :class:`~Auto3D.engines.models.adapter.BaseModelAdapter`'s default and therefore
        every adapter's answer except AIMNet2's.

        This member replaced an ``isinstance(model, AIMNet2Calculator)`` test in
        ``Auto3D.entry.ASE.thermo.vib_hessian``: a third-party type deciding Auto3D's
    ...
    def to_double(self) -> None:
        ...
        ``Auto3D.entry.ASE.thermo.vib_hessian`` builds, and fp32 weights make that fp64
        ...
        only :class:`~Auto3D.engines.models.adapter.BaseModelAdapter` happens to define.
        ...
        Raises:
            NotImplementedError: The model has no meaningful fp64 form. Raising
                ...
                by design -- see :meth:`Auto3D.engines.models.adapter.AIMNet2Adapter.to_double`.
