...
(``Auto3D.engines.isomers.IsomerEngineFactory``) and that path is the public one -- which
...
defines it (``Auto3D.engines.isomers.factory``, ``Auto3D.engines.isomers.base``), so that no name
...
from Auto3D.engines.isomers.factory import IsomerEngineFactory
