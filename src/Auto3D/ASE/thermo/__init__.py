...
``docs/source/api.rst`` documents it (``Auto3D.ASE.thermo.calc_thermo``), which
...
``Auto3D.ASE.thermo.properties`` and friends, which are *children* of
``Auto3D.ASE.thermo``. Records propagate to the parent, so the fifteen-odd tests
that capture on ``logger="Auto3D.ASE.thermo"`` keep seeing warnings raised from
...
from Auto3D.ASE.thermo.driver import calc_thermo
