...
(see ``Auto3D.orchestration.workflow``). They were moved out of ``Auto3D.entry.auto3D`` to break the
...
#
# Auto3D.foundation.utils.logging_config.get_logger(__name__) produces loggers named
# "Auto3D.*" (matching each module's real __name__, e.g. "Auto3D.entry.ASE.thermo").
...
# (Auto3D.orchestration.workflow's self.logger, Auto3D.domain.clash_relief's module logger, the
# stereochemistry-change warning in Auto3D.engines.batch_opt.batchopt). Attaching a
...
def _worker_stdout_to_stderr() -> contextlib.AbstractContextManager[object]:
    ...
    on the inherited fd 1 -- the parent's ``Auto3D.presentation.cli.console.reserve_stdout``
