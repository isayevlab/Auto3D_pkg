...
from where it reached up into ``Auto3D.engines.models`` through two function-scope
...
def check_input(args: Any) -> None:
    ...
    Args:
        args: Arguments object containing Auto3D configuration options.
            ...
            ``Auto3D.foundation.config.FIELD_BOUNDS`` and is enforced when the
    ...
    # Auto3D.foundation.config.FIELD_BOUNDS declared ("ge", 1) -- two minimums for one
...
def check_valid_configuration(options: Auto3DOptions) -> list[str]:
    ...
    *this* machine, whether the engine name resolves in the model registry, and
    ...
    # Auto3D.foundation.config.ENGINE_CHOICES and enforced by Auto3DOptions.__post_init__,
