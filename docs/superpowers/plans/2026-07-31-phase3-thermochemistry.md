...
> **Post-implementation correction (superseded 2026-08-02):** this task originally also planned an optional Truhlar-style “raising” of low-frequency real modes (`low_freq_cutoff_cm`, `VibrationAnalysis.n_raised`, described in Steps 1 and 3 below). It was implemented, removed, and has now been reinstated **on by default**, with `LOW_FREQUENCY_CUTOFF_CM = 100.0`.
>
> The recorded reason for removing it was that raising `analyze_vibrations`’s energies shifts the zero-point and enthalpy sums as well as the entropy, “which is not Truhlar’s method”. **That objection is real in kind but wrong by about 100× in size, and it should not be raised again.** At 298 K a mode below the cutoff is deep in the classical limit (`kT = 207 cm⁻¹`) and carries ≈RT of `ZPE + ΔH_vib(0→T)` regardless of its frequency: 0.043 + 0.551 = 0.594 kcal/mol at 30 cm⁻¹ against 0.143 + 0.461 = 0.604 at 100 cm⁻¹ — the zero-point rise (+0.100) is cancelled by the thermal-enthalpy fall (−0.090). Raising everywhere therefore differs from raising inside the entropy alone by **0.010–0.012 kcal/mol per mode** (0.029 kcal/mol for all of n-decane), while the defect it bounds is worth 1.6 kcal/mol on the same molecule. Ribeiro et al.’s own wording (“all vibrational frequencies lower than 100 cm⁻¹ raised to 100 cm⁻¹”) does not restrict the substitution to the entropy either.
>
> The current interface is `analyze_vibrations(vib_energies, n_atoms, geometry, *, imag_cutoff_cm=..., low_freq_cutoff_cm=...)`, taking exactly the `3N−6`/`3N−5` **projected** vibrations (see `projected_vibrations`), and `VibrationAnalysis` carries `n_raised`, `n_removed` and `convention`. The code and test snippets below are left as originally written for the historical record and do not reflect the current interface.
